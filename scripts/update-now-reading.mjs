...
const args = parseNamedArgs([
    'id', 'title', 'author', 'cover', 'link', 'progress', 'rating',
    'summary', 'review', 'year', 'genre',
]);

if (args.has('id') && args.has('title')) {
    throw new Error('Use --id to select an existing book or --title to add a new one, not both.');
}
...
let selectedId = current.readingId;
let addedItem = null;
...
if (args.has('id')) {
    ...
    if (!author || !coverInput) throw new Error('A new book requires --title, --author, and --cover.');

    selectedId = createUniqueId('book', args.get('title'), existingIds);
    addedItem = {
        id: selectedId,
        type: 'book',
...
if (!selectedId) throw new Error('No current reading item is configured.');

const overrideFields = ['link', 'summary', 'review', 'year', 'genre'];
...
console.log(`Now Reading points to ${selectedId}`);
