...
const baseUrl = `https://api.football-data.org/v4/teams/${TEAM_ID}/matches?status=FINISHED&limit=1`;
...
function fetchScores(url, isFallback = false) {
            ...
            try {
                ...
                if (!json.matches || json.matches.length === 0) {
                    if (!isFallback) {
                        ...
                        console.log(`No matches found in current default season. Trying previous season (${prevSeason})...`);
                        fetchScores(`${baseUrl}&season=${prevSeason}`, true);
                    } else {
                        console.error("No matches found in previous season either.");
                        process.exit(0); // Don't fail, just don't update
                    }
                    return;
                }

                saveMatch(json.matches[0]);
